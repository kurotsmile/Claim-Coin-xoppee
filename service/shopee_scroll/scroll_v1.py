    ...
    def claim_coin(self):
        ...
        while not self.is_stop:
            ...
            if self.go_to_link is not None:
                ...
                time.sleep(45)
                self.go_to_link = None
            if not self.d.xpath(self.COIN_STATE).wait(20):
            ...
            self.send_log(f'Status: ({str(coin_status)})')
            if coin_status == None or coin_status == '' or coin_status == 'Kết thúc' or coin_status == 'Giới hạn':
            ...
            if coin_status == 'Đăng nhập':
                self.update_stop_status(True, 'Đăng nhập Setp1')
                break

            if not self.d.xpath(self.COIN_NUM).wait(1):
                time.sleep(self.random_sleep)
                continue
            
            coin_value = self.get_text_xpath(self.COIN_NUM)
      
            if coin_value == None or int(coin_value) < int(self.min_coin):
                time.sleep(self.random_sleep)
                continue  
            
            time_get_coin = coin_status[:2] 
            if(time_get_coin.isdigit() and int(time_get_coin) > int(self.max_minute)):
                time.sleep(self.random_sleep)
                continue
            #print('Coin Value')
            ...
            while not self.is_stop:
                ...
                # if self.d.app_current()['package'] != self.APP_PACKAGE:
                    # self.start_app('Step 2 APP_PACKAGE')
                    # break
                self.click_exist(self.POPUP_AD,1)
                if self.go_to_link is not None:
                    self.open_link()
                ...
                if int(coin_value) < int(self.min_coin):
                    time.sleep(1)
                    break  
                time_get_coin = current_status[:2] 
                if(time_get_coin.isdigit() and int(time_get_coin) > int(self.max_minute)):
                    time.sleep(1)
                    break
                ...
                if self.get_text_xpath(self.COIN_STATE) == 'Lưu':
                    ...
                    coin_status = self.get_text_xpath(self.COIN_STATE)
                    if coin_status == None or coin_status == '' or coin_status == 'Kết thúc' or coin_status == 'Giới hạn' or coin_status == 'Đăng nhập':
                        time.sleep(1)
                        break
